...
const opSchemas = [
  ...
  z
    .object({
      ...
      presentation: z.enum(TRANSITION_PRESENTATIONS).optional(),
      direction: z.enum(['from-left', 'from-right', 'from-top', 'from-bottom']).optional(),
      alignment: finite.min(0).max(1).optional(),
      durationInFrames: positiveFrames.optional(),
    })
    .strict(),
  z
  ...
  return {
    ...
    semantics: {
      transform: {
        ...
        movingStickerRequiresDistinctPositionKeyframes: true,
        opacityIsNotPositionMotion: true,
      },
      transitions: {
        baseType: 'crossfade',
        presentationField: 'presentation',
        presentations: TRANSITION_PRESENTATIONS,
        requiresAdjacentClipsWithSourceHandles: true,
        gpuEffectsAreNotClipPointTransitions: true,
      },
      audio: {
        trackMuteField: 'muted',
        volumeField: 'volume',
        volumeUnit: 'dB',
        unityGainDb: 0,
        recommendedTopology: 'source audio and background music on separate audio tracks',
        sourceMuteMethod: 'set muted=true on the source-audio track',
        masterBusIsNotSourceMute: true,
      },
    },
    schemas: {
...
export function compactCapabilities() {
  ...
  return {
    ...
    canonicalCommands: {
      ...
      renderSubmit: commands['render submit'],
      renderStatus: commands['render status'],
    ...
    semantics: {
      ...
      movingStickerExample: [
        {
          callerId: 'stickerStart',
          op: 'addKeyframe',
          itemId: 'sticker-item-id',
          property: 'x',
          frame: 0,
          value: -320,
        },
      ...
      transitionPresentations: TRANSITION_PRESENTATIONS,
      audioVolumeUnit: 'dB',
      zeroDbMeaning: 'unity gain, not mute',
      sourceMuteField: 'track.muted',
    },
